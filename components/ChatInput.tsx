...
export default function ChatInput({
  ...
  // Memoized function to send typing status
  const sendTypingStatus = useCallback(
    (isTyping: boolean) => {
      console.log("🔧 ChatInput: sendTypingStatus called with:", isTyping);
      onTyping?.(isTyping);
    },
    [onTyping]
  );
    ...
    (currentText: string) => {
      ...
      if (hasText) {
        ...
        if (!isCurrentlyTyping.current) {
          console.log("🔧 ChatInput: Starting typing indicator");
        ...
        typingTimeoutRef.current = setTimeout(() => {
          if (isCurrentlyTyping.current) {
            console.log("🔧 ChatInput: Stopping typing indicator (timeout)");
        ...
        if (isCurrentlyTyping.current) {
          console.log("🔧 ChatInput: Stopping typing indicator (empty text)");
          isCurrentlyTyping.current = false;
          sendTypingStatus(false);
        }
      }
  ...
  const handleTextChange = useCallback(
    (newText: string) => {
      setText(newText);
      handleTypingLogic(newText);
    },
    [handleTypingLogic]
  );

  // Stop typing when component unmounts or user leaves
  useEffect(() => {
    return () => {
      if (isCurrentlyTyping.current) {
        sendTypingStatus(false);
  ...
  const handleSend = async () => {
    ...
    console.log("🔧 ChatInput: Sending message, stopping typing");
